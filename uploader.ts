...
export class FileUploadService {
  ...
  public getStorage(destination: string): StorageEngine {
    if (!fs.existsSync(`uploads/${destination}`)) {
      fs.mkdirSync(`uploads/${destination}`, { recursive: true });
    }

    return multer.diskStorage({
      destination: (_req: any, _file: any, cb: (arg0: null, arg1: string) => any) =>
  ...
  public uploader(
    destination: string,
    limits: Record<string, number> = { fieldSize: 1e8, fileSize: 10 * 1024 * 1024/* 1e7 */, files: 1 }
  ): Multer {
    return multer({
